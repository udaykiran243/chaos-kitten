...
class PoCGenerator:
    ...
    def _generate_template(self, finding: Dict[str, Any]) -> str:
        ...
        is_race = "race" in vuln_type.lower() or "race" in str(finding.get("type", "")).lower()
        ...
        if is_race:
            return textwrap.dedent(f"""\
                #!/usr/bin/env python3
                \"\"\"Proof-of-Concept: {safe_vuln} (Race Condition)
                ...
                async def send_request(client, idx):
                    try:
                        ...
                        print(f"[*] Request {{idx}} status: {{resp.status_code}}")
                        return resp
                ...
                async def main() -> None:
                    print(f"[*] Starting concurrent PoC for {safe_vuln}")
                    async with httpx.AsyncClient(timeout=30) as client:
                        tasks = [send_request(client, i) for i in range(CONCURRENCY)]
                        responses = await asyncio.gather(*tasks)
                    
                    successes = [r for r in responses if r and 200 <= r.status_code < 300]
                    print(f"[*] Successfully executed {{len(successes)}}/{{CONCURRENCY}} requests.")
